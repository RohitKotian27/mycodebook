...
import { getUserData, createUserOrder } from "../../../services";
...
export const Checkout = ({ setCheckout }) => {
    ...
    useEffect(() => {
        async function fetchUserData() {
            try {
                const requiredData = await getUserData();
                setUser(requiredData);
            } catch (error) {
                toast.error(`User Data : ${error.message}`, { position: "bottom-center" });
            }
        };
        fetchUserData();
    }, [token, cbid]);
                        ...
                        <div className="py-6 px-6 lg:px-8">
                            ...
                            <form onSubmit={handleOrderSubmit} className="space-y-6" >
                                <div>
                                    ...
                                    <input type="text" name="name" value={user.name || ''} id="name" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:value-gray-400 dark:text-white" disabled required="" />
                                </div>
                                <div>
                                    <label htmlFor="email" className="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">Email:</label>
                                    <input type="text" name="email" value={user.email || ''} id="email" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:value-gray-400 dark:text-white" disabled required="" />
                                </div>
                                <div>
