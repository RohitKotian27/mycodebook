...
export const Search = ({setSearchToggle}) => {
    const navigate = useNavigate();
    const searchQuery = useRef();
    const handleSubmit = (event) => {
        event.preventDefault();
        setSearchToggle(false);
        navigate(`/products?q=${searchQuery.current.value}`);
    };
        ...
        <div className="mx-auto max-w-screen-xl p-2 my-5">
            <form onSubmit={handleSubmit} className="flex items-center">
                <div className="relative w-full">
                    ...
                    <input ref={searchQuery} name="search" type="text" id="simple-search" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Search" autoComplete="off" required="" />
                </div>
